...
public abstract class AbstractNetworkMerge implements NetworkMerge {
     ...
     private boolean interrupted; // to enable cancel of the network merge operation
    ...
    public CyNetwork mergeNetwork(final CyNetwork toNetwork, final List<CyNetwork> fromNetworks, final Operation op) {
        ...
        //List<CyNode> nodes = new ArrayList<CyNode>(nNode);
        ...
        updateTaskMonitor("Successfully merged the selected "+fromNetworks.size()+" networks into network "+toNetwork.getSUID()+" with "+nNode+" nodes and "+nEdge+" edges",1.0);
    ...
    private <T extends CyTableEntry> List<Map<CyNetwork,Set<T>>> getMatchedList(final List<CyNetwork> networks, final boolean isNode, Set<Set<CyNode>> matchedNodes) {
        ...
        for (int i=0; i<nNet; i++) {
            ...
            final Iterator<T> it;
            if (isNode) {
            	it = (Iterator<T>)net1.getNodeList().iterator(); 
            } else { //edge
            	it = (Iterator<T>)net1.getEdgeList().iterator();
            }
