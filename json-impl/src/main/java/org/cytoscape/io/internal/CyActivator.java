...
public class CyActivator extends AbstractCyActivator {
	...
	public void start(BundleContext bc) {
		...
		final CyFileFilter cytoscapejsReaderFilter = new CytoscapejsFileFilter(
				...
				"Cytoscape.js JSON format", DataCategory.NETWORK, streamUtil);
		...
		final BasicCyFileFilter cytoscapejsFilter = new BasicCyFileFilter(
				new String[] { "json" }, new String[] { "application/json" },
				"Cytoscape.js JSON format", DataCategory.NETWORK, streamUtil);
		final BasicCyFileFilter d3jsFilter = new BasicCyFileFilter(
				new String[] { "json" }, new String[] { "application/json" },
				"D3.js JSON format", DataCategory.NETWORK, streamUtil);
		final BasicCyFileFilter d3jsTreeFilter = new BasicCyFileFilter(
				new String[] { "json" }, new String[] { "application/json" },
				"D3.js Tree JSON files", DataCategory.NETWORK, streamUtil);

		final BasicCyFileFilter vizmapJsonFilter = new BasicCyFileFilter(
				new String[] { "json" }, new String[] { "application/json" },
				"Cytoscape.js Visual Style JSON format", DataCategory.VIZMAP,
				streamUtil);

		final JSONNetworkWriterFactory jsonWriterFactory = new JSONNetworkWriterFactory(
				cytoscapejsFilter, jsMapper);
		registerAllServices(bc, jsonWriterFactory, new Properties());

		final JSONNetworkWriterFactory d3jsWriterFactory = new JSONNetworkWriterFactory(
				d3jsFilter, d3jsMapper);
		registerAllServices(bc, d3jsWriterFactory, new Properties());

		final JSONNetworkWriterFactory d3jsTreeWriterFactory = new JSONNetworkWriterFactory(
				d3jsTreeFilter, d3jsTreeMapper);
		registerAllServices(bc, d3jsTreeWriterFactory, new Properties());

		final JSONVisualStyleWriterFactory jsonVSWriterFactory = new JSONVisualStyleWriterFactory(
				vizmapJsonFilter, jsMapper);
