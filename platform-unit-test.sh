...
# Toolchain VERSION from the impl pom (XML parse; Maven can't evaluate the pom until its parent is
# installed below), and the matching git REF. VER != CY_REF for SNAPSHOTs: a 3.11.0-SNAPSHOT dev
# build resolves 3.11.0-SNAPSHOT artifacts cloned from the parent/api 'develop' branch.
...
case "$VER" in *-SNAPSHOT) CY_REF="${CY_REF:-develop}" ;; *) CY_REF="${CY_REF:-$VER}" ;; esac
echo "Cytoscape api version: $VER ; toolchain ref: $CY_REF ; local repo: $LOCAL_REPO"

# Build parent + api into $LOCAL_REPO when absent (skipped once present; guard on the VERSION, not the
# git ref). These use a pom OUTSIDE the impl repo (-f), so Maven derives the .mvn base dir from that
# pom's location, not cwd — .mvn/maven.config does NOT apply to them. Pass the settings explicitly.
if [ ! -f "$LOCAL_REPO/org/cytoscape/api-bundle/$VER/api-bundle-$VER.jar" ]; then
  work="$(mktemp -d)"
