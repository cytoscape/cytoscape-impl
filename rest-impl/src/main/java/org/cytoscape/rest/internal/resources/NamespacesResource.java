...
public class NamespacesResource implements RESTResource, PaxAppender, TaskObserver {
	...
	/**
	 * Method handling HTTP GET requests to enumerate all namespaces. 
	 * The returned list will be sent to the client as "text/plain" media type.
	 *
	...
	/**
	 * Method handling HTTP GET requests to enumerate all namespaces. 
	 * The returned list will be sent to the client as "text/html" media type.
	 *
	...
	/**
	 * Method to enumerate all arguments for a given namespace and command
	 * or execute a namespace and command if query strings are provided
	 * @param namespace
	 * @param command
	 * @param uriInfo this provides access to the query strings
	 * @return list of arguments as text/plain or the results of executing the command
	 */
	@GET @Path("/{namespace}/{command}")
	@Produces(MediaType.TEXT_PLAIN)
	 ...
	 * @param command
	 * @param uriInfo this provides access to the query strings
	 * @return list of arguments as text/html or the results of executing the command
	 */
	@GET @Path("/{namespace}/{command}")
	@Produces(MediaType.TEXT_HTML)
	...
	private String handleCommand(String namespace, String command,
	...
	private String executeCommand(String namespace, String command, 
	                              MultivaluedMap<String, String> args, MessageHandler handler) {
    ...
    for (String inputArg: args.keySet()) {
      ...
      if (!found) {
		...
		taskManager.execute(
			ceTaskFactory.createTaskIterator(namespace, command, modifiedSettings, this), this);
