...
public class StartTask extends ImportNetworkFromPublicDataSetTask {
	...
	public StartTask(final QuickStartState state, final ImportTaskUtil importTaskUtil,
		...
		super(importTaskUtil.processors, importTaskUtil.mgr, networkManager, importTaskUtil.networkViewManager,
				importTaskUtil.props, importTaskUtil.cyNetworkNaming, importTaskUtil.streamUtil);
		this.state = state;
	...
	public void run(TaskMonitor tm) throws Exception {
		if((go != null && go.trim().length() != 0)|| (phynotype != null && phynotype.trim().length() !=0)) {
			logger.info("GO = " + go);
			logger.info("Phynotype = " + phynotype);
			insertTasksAfterCurrentTask(new SearchRelatedGenesTask(subnetworkUtil, new SubnetworkBuilderState(), go, phynotype));
		}
		...
		if(queryGenes != null && queryGenes.trim().length() != 0) {
			...
			insertTasksAfterCurrentTask(new CreateSubnetworkWithoutGeneListTask(subnetworkUtil, new SubnetworkBuilderState(), queryGenes, selectedType));
		}
