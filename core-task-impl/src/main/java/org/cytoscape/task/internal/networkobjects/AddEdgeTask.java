...
public class AddEdgeTask extends AbstractTask implements ObservableTask {
	...
	@Tunable(
			description="Name of edge source node", 
	...
	@Tunable(
			description="Name of edge target node",
			longDescription="Enter the name of an existing node in the network "+
					        "to be the target of the edge.  Note that this is the "+
							"name as defined in the 'name' column of the network", 
			exampleStringValue="Node 2",
			context="nogui",
			required=true
	)
	...
	public void run(final TaskMonitor tm) {
		...
		if (source == null) {
			tm.showMessage(TaskMonitor.Level.ERROR, "Can't find source node named '"+sourceName+"'");
			return;
		}

		if (target == null) {
			tm.showMessage(TaskMonitor.Level.ERROR, "Can't find target node named '"+targetName+"'");
			return;
		}
		...
		eventHelper.flushPayloadEvents();
		tm.showMessage(TaskMonitor.Level.INFO, "Added edge "+newEdge.toString()+" to network");
	}
