...
public class FileCLHandler extends AbstractCLHandler{
	 ...
	 */
	public FileCLHandler(Method gmethod, Method smethod, Object o, Tunable t) {
	...
	public void handleLine(CommandLine line) {
		...
		try {
			if (line.hasOption(fc)) {
				...
				File file = new File(line.getOptionValue(fc));
				setValue(file);
			}
	...
	public Option getOption() {
		...
		try {
			currentValue = (File)getValue();
		} catch(final Exception e) {
			e.printStackTrace();
			System.exit(1);
		}

		final String path = (currentValue == null) ? "" : currentValue.getAbsolutePath();
	 ...
	 */
	private void displayCmds(final String fc) {
