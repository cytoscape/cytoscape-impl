...
public class BrowserTableTest {
	...
	public BrowserTableTest() {
		...
		when(registrar.getService(EquationCompiler.class)).thenReturn(equationCompiler);
		when(registrar.getService(DialogTaskManager.class)).thenReturn(taskManager);
		when(registrar.getService(OpenBrowser.class)).thenReturn(openBrowser);
		when(registrar.getService(CyApplicationManager.class)).thenReturn(applicationManager);
		when(registrar.getService(IconManager.class)).thenReturn(iconManager);
		when(registrar.getService(CyEventHelper.class)).thenReturn(eventHelper);
		when(registrar.getService(CyTableManager.class)).thenReturn(tableManager);
...
//		btcm.setAllColumnsVisible();
	...
	void createTable() {
		sharedTable = new CyTableImpl("shared table", CyNetwork.SUID, Long.class, false, true, SavePolicy.DO_NOT_SAVE,
...
//	}
	...
	public void testTableAutoMode() {
		...
		browserTable.handleEvent(new RowsSetEvent(table, Arrays
				.asList(new RowSetRecord(table.getRow((long) 3), CyNetwork.SELECTED, (Object) true, (Object) true))));
		assertEquals(1, browserTable.getRowCount());

		table.getRow((long) 2).set(CyNetwork.SELECTED, true);
		browserTable.handleEvent(new RowsSetEvent(table, Arrays
				.asList(new RowSetRecord(table.getRow((long) 2), CyNetwork.SELECTED, (Object) true, (Object) true))));
		assertEquals(2, browserTable.getRowCount());

		table.getRow((long) 1).set(CyNetwork.SELECTED, true);
		browserTable.handleEvent(new RowsSetEvent(table, Arrays
				.asList(new RowSetRecord(table.getRow((long) 1), CyNetwork.SELECTED, (Object) true, (Object) true))));
		assertEquals(3, browserTable.getRowCount());
		...
		browserTable.handleEvent(new RowsSetEvent(table,
				Arrays.asList(
	...
	public void testTableModeAndSelection() {
		...
		browserTable.mouseReleased(new MouseEvent(browserTable, 0, 0, 0, browserTable.getCellRect(1, 0, true).x,
				browserTable.getCellRect(1, 0, true).y, Math.abs(browserTable.getCellRect(1, 0, true).x),
				Math.abs(browserTable.getCellRect(1, 0, true).y), 1, true, MouseEvent.BUTTON1));
		assertEquals(1, browserTable.getSelectedRowCount());
