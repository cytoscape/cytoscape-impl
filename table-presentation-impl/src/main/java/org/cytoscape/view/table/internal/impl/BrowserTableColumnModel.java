...
public class BrowserTableColumnModel extends DefaultTableColumnModel {
	...
	public void setColumnVisible(TableColumn column, boolean visible) {
		if (!visible) {
			...
			// iterate through both collections of visible and all columns,
			// counting
	...
	public boolean setColumnFormat(String column, String format) {
		if (format == null){
			if(columnFormatMap.containsKey(column))
				columnFormatMap.remove(column);
			return true;
		} else {
			try {
				String.format(format, 1.2);
				columnFormatMap.put(column, format);
				return true;
	...
	public String getColumnFormat(String colName) {
		return columnFormatMap.get(colName);
	}
	 ...
	 * @exception IllegalArgumentException
	 *                if either <code>oldIndex</code> or <code>newIndex</code>
	 ...
	 *
	 * @param onlyVisible
	 *            if set all invisible columns will be missing from the
	 *            enumeration.
	 ...
	 * @param onlyVisible
	 *            if set columnIndex is meant to be relative to all visible
