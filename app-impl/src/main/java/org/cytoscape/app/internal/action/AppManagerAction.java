 ...
 *
 */
public class AppManagerAction extends AbstractCyAction implements SessionLoadedListener {
	 ...
	 */
	public AppManagerAction(CySwingApplication desktop,
							CyVersion version,
		...
		this.bookmarks = bookmarksProp.getProperties();	
		...
		DefaultAppUrl = cytoscapeProperties.getProperties().getProperty("defaultPluginDownloadUrl");
	...
	public void actionPerformed(ActionEvent e) {
		AppManageDialog dlg = new AppManageDialog(desktop.getJFrame(), bookmarks, this.bookmarksUtil, 
				this.dialogTaskManager, this.appLoaderTaskFactory);
		...
		if (sess != null) {
			for (CyProperty<?> p : sess.getProperties()) {
				...
				if (obj instanceof Bookmarks) {
					...
					newBookmarks = (Bookmarks) obj;
					break;
				}
			}
		}
		
		this.bookmarks = newBookmarks != null ? newBookmarks : new Bookmarks();
	}
