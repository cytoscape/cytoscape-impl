 ...
 * 
 */
public class InstallableApp implements Installable {
	...
	//private static final Logger logger = LoggerFactory.getLogger(InstallableApp.class);

	private AppInfo infoObj;

	public InstallableApp(AppInfo obj) {
		this.infoObj = obj;
	}

	public AppInfo getInfoObj() {
		return this.infoObj;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cytoscape.app.Installable#install()
	 */
	public boolean install() throws java.io.IOException,
			org.cytoscape.app.internal.ManagerException {
	...
	public boolean installToDir(File dir, TaskMonitor taskMonitor) 
		...
		File AppDir = dir;
		if (AppDir == null) 
			AppDir = infoObj.getAppDirectory();
		else
			AppDir = new File(AppDir, getInfoObj().getName() + "-" + getInfoObj().getObjectVersion());
		
		if (!AppDir.exists()) {
			AppDir.mkdirs();
		}

		File Download = new File(AppDir, createFileName(infoObj));
		if (taskMonitor != null)
...
//		try {
//
//			String ClassName = JarUtil.getAppClass(Download.getAbsolutePath(),
//					infoObj.getFileType());
//			if (ClassName != null) {
//				infoObj.setAppClassName(ClassName);
//			} else { // no class name so delete the app
//				Download.delete();
//				Download.getParentFile().delete();
//				ManagerException E = new ManagerException(
//						infoObj.getName()
//								+ " does not define the attribute 'Cytoscape-App' in the jar manifest file.\n"
//								+ "This app cannot be auto-installed.  Please install manually or contact the app author.");
//				throw E;
//			}
...
//		}
		...
		infoObj.setInstallLocation(AppDir.getAbsolutePath());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cytoscape.app.Installable#uninstall()
	 */
	public boolean uninstall() throws org.cytoscape.app.internal.ManagerException {
		...
		if (infoObj.getInstallLocation() != null
			...
			Deleted = AppManager.recursiveDeleteFiles(Installed);
		} else {
	...
	public java.util.List<org.cytoscape.app.internal.DownloadableInfo> findUpdates()
		...
		final List<DownloadableInfo> UpdatableApps = new ArrayList<DownloadableInfo>();
		...
		final AppInfo InfoToUpdate = this.infoObj;
		final List<Exception> Exceptions = new ArrayList<Exception>();

		AppManagerInquireTask task = new AppManagerInquireTask(
				this.infoObj.getDownloadableURL(), new AppInquireAction() {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cytoscape.app.Installable#update()
	 */
	...
	public boolean update(DownloadableInfo newInfoObj,
		...
		if ((infoObj.getID().equals(newObj.getID()) || infoObj
			...
			throw new ManagerException(
					"Failed to update '"
							+ infoObj.getName()
							+ "', the new app did not match what is currently installed\n"
	...
	/*
	 * Standard name for the app install file <App Name>.jar|zip
	 */
	private String createFileName(AppInfo Obj) {
