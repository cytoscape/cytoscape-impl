...
//		addMouseMotionListener(new CanvasMouseMotionListener());
	...
	private boolean isAnnotationSelectionMode() {
...
//	}
	...
	private class CanvasKeyListener extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			boolean annotationSelectionMode = isAnnotationSelectionMode();
			
			if(annotationSelectionMode && annotationSelection.count() > 0 && (code == VK_UP || code == VK_DOWN || code == VK_LEFT || code == VK_RIGHT)) {
				moveAnnotations(e);
			} else if(annotationSelectionMode && code == VK_ESCAPE) {
				cancelAnnotations();
			} else {
				networkCanvasKeyPressed(e);
			}
		}
		...
		public void keyReleased(KeyEvent e) { 
			...
			if(isAnnotationSelectionMode() && annotationSelection.count() > 0 && code == VK_DELETE) {
		...
		private void networkCanvasKeyPressed(KeyEvent k) {
			...
			else if ( code == VK_BACK_SPACE ) 		//#1993
				handleBackspaceKey();
		}
