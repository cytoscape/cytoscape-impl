...
public class mBoundedAnnotationPanel extends javax.swing.JPanel {

	    public mBoundedAnnotationPanel(BoundedAnnotation mAnnotation) {
	    ...
	    private void initComponents() {
	        ...
	        for(int i=0;i<bFTList.getModel().getSize();i++){
	            
	            if(mAnnotation.getFont().getFamily().equals((String)bFTList.getModel().getElementAt(i))){
	            	bFTList.setSelectedIndex(i);
	                break;
	            }
	        }
	        ...
	        bFTField.setText((String)bFTList.getSelectedValue());
	        add(bFTField);
	        ...
	        if(mAnnotation.getFont().getStyle()==Font.PLAIN)
	        	...
	        	bFSList.setSelectedIndex(1);

	        else if(mAnnotation.getFont().getStyle()==Font.ITALIC) 
	        	bFSList.setSelectedIndex(2);                    
	        
	        else
	        	bFSList.setSelectedIndex(3);	
	        ...
	        bFSField.setText((String)bFSList.getSelectedValue());
	        add(bFSField);
	        bFSField.setBounds(184, 103, 110, 20);

	        bAnnotationText.setText(mAnnotation.getText());
	        ...
	        for(i=0;i<bFSizeList.getModel().getSize();i++){

	            if(fontSize==Integer.parseInt((String)bFSizeList.getModel().getElementAt(i)) ){
	            	bFSizeList.setSelectedIndex(i);
	                break;
	            }
	        }

	        if(i==bFSizeList.getModel().getSize())
	        	bFSizeList.setSelectedIndex(2);
	        ...
	        bFSizeField.setText((String)bFSizeList.getSelectedValue());
	        add(bFSizeField);
	        ...
	        bSList.setSelectedIndex(mAnnotation.getShapeType());
	        ...
	        int eT=(int)mAnnotation.getEdgeThickness();
	        
	        bEdgeThickness.setSelectedIndex(1);
	        		        
	        for(i=0;i<bEdgeThickness.getModel().getSize();i++){

	            if( eT==Integer.parseInt((String)bEdgeThickness.getModel().getElementAt(i)) ){
	            	bEdgeThickness.setSelectedIndex(i);
	                break;
	            }
	        }
	        
	        add(bEdgeThickness);
	        bEdgeThickness.setBounds(312, 382, 37, 20);
	    ...
	    private void bSTCButtonActionPerformed(java.awt.event.ActionEvent evt) {
	        ...
	        SelectColor bASelectColor=new SelectColor(preview, 0, this.jPanel1, mAnnotation.getTextColor());
	    ...
	    private void bSECButtonActionPerformed(java.awt.event.ActionEvent evt) {
	        ...
	        SelectColor bASelectColor=new SelectColor(preview, 2, this.jPanel1, mAnnotation.getEdgeColor());
	    ...
	    private void bSFCButtonActionPerformed(java.awt.event.ActionEvent evt) {
	        ...
	        SelectColor bASelectColor=new SelectColor(preview, 1, this.jPanel1, mAnnotation.getFillColor());
	        
            bASelectColor.setVisible(true);
            bASelectColor.setSize(435, 420);
	    }

	    public BoundedAnnotation getPreview(){
